...
$sql = "SELECT * FROM parent WHERE parent_id = ".$parent_id." AND (book_id = ".$book_id." OR book_id != 0)";
$result = $mysqli->query($sql);
if($result->num_rows > 0){
	$_SESSION['message'] = 'This parent has already checked out a book';
	...
	if($result->num_rows == 0){
		...
		if($result->num_rows == 0){
			...
			if($book_id > 100){
				$_SESSION['message'] = 'Parents do not have access level to this book';
				header("location: dberror.php");
			}
			else{
				...
				if($due_date === '2099-12-31'){	//meaning available
					// rent it
					$sql = "UPDATE parent SET book_id = ".$book_id." WHERE parent_id = ".$parent_id;
